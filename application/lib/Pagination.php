...
class Pagination {
    ...
    public function get() {
        ...
        if (!is_null($links)) {
            if ($this->current_page > 1) {
                $links = $this->generateHtml($this->current_page, 'Вперед', -1).$links;
            }
            if ($this->current_page < $this->amount) {
                $links .= $this->generateHtml($this->current_page, 'Назад', 1);
            }
        }
        $html .= $links.' </ul></nav>';
        return $html;
    ...
    private function generateHtml($page, $text = null, $direct = 0) {
        if (!$text) {
            $text = $page;
        }
        $page += $direct;
